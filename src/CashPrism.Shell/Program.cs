...
// The generic host logs a failed start with the full stack trace before the
// exception reaches the catch below. That trace is duplicate information in a
// window someone opened by double-clicking: anything this code does not handle
// still surfaces through the runtime's unhandled-exception output.
...
// straight from configuration here. Everything after Build() uses IOptions.
...
// option. HTTP only and on purpose: a self-signed certificate means a warning on
// every phone and tablet. The consequence to carry into the shared password is
// that credentials travel the home network unencrypted.
...
catch (IOException exception) when (exception.InnerException is AddressInUseException)
{
    ...
    Console.Error.WriteLine($"CashPrism cannot start: port {hosting.Port} is already in use.");
    Console.Error.WriteLine("Stop the other instance, or pick another port with --port <number>.");
